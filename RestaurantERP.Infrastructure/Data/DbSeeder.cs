...
public class DbSeeder
{
    ...
    public async Task SeedAsync()
    {
        ...
        await SeedMissingMenuAccessAsync();
    ...
    private async Task SeedMenuItemsAsync()
    {
        ...
        var beverages = categories.FirstOrDefault(c => c.Name == "Beverages") ?? categories.Last();
    ...
    private async Task SeedTablesAsync()
    {
        ...
        var ground = new TableSection { Name = "Ground Floor", Description = "Main dining area" };
        var rooftop = new TableSection { Name = "Rooftop", Description = "Outdoor seating" };
        _context.TableSections.AddRange(ground, rooftop);
        await _context.SaveChangesAsync();
    ...
    {
        if (await _context.RoleMenuAccesses.AnyAsync()) return;

        foreach (var role in AdminMenuSections.ManageableRoles)
        {
            foreach (var section in AdminMenuSections.All)
            {
                var canRead = role == "Admin" || (role == "Kitchen" && section.Key is AdminMenuSections.Orders or AdminMenuSections.MenuItems)
                    || (role == "Delivery" && section.Key == AdminMenuSections.Orders);

                var canWrite = role == "Admin" || (role == "Kitchen" && section.Key == AdminMenuSections.Orders);
    ...
    {
        if (await _context.ThemeSettings.AnyAsync()) return;
    ...
    {
        if (await _context.CommissionRules.AnyAsync()) return;
    ...
    {
        if (await _context.NavigationMenus.AnyAsync()) return;

        var mainMenu = new NavigationMenu { Name = "Main Navigation", MenuKey = "main", IsActive = true };
        _context.NavigationMenus.Add(mainMenu);

        var footerMenu = new NavigationMenu { Name = "Footer Quick Links", MenuKey = "footer", IsActive = true };
    ...
    private async Task SeedMissingMenuAccessAsync()
    {
        foreach (var role in AdminMenuSections.ManageableRoles)
        {
            foreach (var section in AdminMenuSections.All)
